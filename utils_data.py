...
class ScienceQADatasetImg(Dataset):
    ...
    def random_masking(self, input_ids, attention_mask, mask_rate=0.2):
        """
        Randomly mask tokens in the input sequence for data augmentation.
        """
        ...
        num_tokens_to_mask = int(mask_rate * masked_input_ids.numel())
        random_tokens_to_mask = torch.randperm(masked_input_ids.numel())[:num_tokens_to_mask]
        masked_input_ids[0][random_tokens_to_mask] = self.tokenizer.pad_token_id
    ...
    def __getitem__(self, index):
        ...
        source_ids_masked = source["input_ids"].squeeze()
        source_mask_masked = source["attention_mask"].squeeze()

        source_ids_masked = self.random_masking(source_ids_masked, source_mask_masked)

        
        return {
            "input_ids": source_ids,
            "input_ids_masked": source_ids_masked,
            "attention_mask": source_mask,
            "image_ids": image_ids,
            "labels": target_ids,
        }